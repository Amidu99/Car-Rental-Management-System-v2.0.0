...
public class DashboardFormController implements Initializable {
    ...
    public static void loadCurrentValues() {
        try {
            platinumCount = dashboardBO.getMembCount("M1");
            goldCount = dashboardBO.getMembCount("M2");
            silverCount = dashboardBO.getMembCount("M3");
            carCount = dashboardBO.getVehicleCount("car");
            vanCount = dashboardBO.getVehicleCount("van");
            lorryCount = dashboardBO.getVehicleCount("lorry");
    ...
    public void getAllAvailableVehicles(LocalDate rideDate) {
        try {
            ...
            List<VehicleDTO> vehicleList = dashboardBO.getAllAvailableVehicles(rideDate);
            for (VehicleDTO vehicleDTO : vehicleList) {
                obList.add(new VehicleTM(
                        vehicleDTO.getVehiNo(),
                        vehicleDTO.getVehiType(),
                        vehicleDTO.getModel(),
                        vehicleDTO.getCostPerKM(),
    ...
    public void btnEnterAdminPassOnAction(ActionEvent actionEvent) {
        ...
        if (!password.isEmpty()) {
            ...
            try {
                isAdmin = dashboardBO.isAdmin(password);
            } catch (SQLException e) {
                checkforAdminPane.setVisible(false);
    ...
    public void setVehiImage(String vehiNo, String vehiModel) {
        try {
            InputStream inputStream = dashboardBO.getImage(vehiNo);
            if (inputStream != null) {
    ...
    public void btnReport4OnMouseExit(MouseEvent event) {
        lblReport4.setVisible(false);
    }
}
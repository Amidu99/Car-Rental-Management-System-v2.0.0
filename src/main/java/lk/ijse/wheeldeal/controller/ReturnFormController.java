...
public class ReturnFormController implements Initializable {
    ...
    private void generateNextReturnNo() {
        try {
            String nextNo = returnBO.generateNextReturnNo();
    ...
    private void loadRideNos() {
        try {
            ...
            List<String> rideNos = returnBO.getPendingRideNos();
    ...
    private void getLast20Returns() {
        try {
            ...
            List<ReturnDTO> returnDTOList = returnBO.getLast20Returns();
            for(ReturnDTO aReturnDTO : returnDTOList) {
                obList.add(new ReturnTM(
                        aReturnDTO.getReturnNo(),
                        aReturnDTO.getRideNo(),
                        aReturnDTO.getDistance(),
                        aReturnDTO.getCost(),
                        aReturnDTO.getReDate()
                ));
            }
    ...
    public boolean checkMultiple(){
        int vehiCount;
        try {
            vehiCount = returnBO.getCount(cmbRideNo.getValue());
            ...
            if (cmbRideNo.getValue() != null && !txtDistance.getText().isEmpty()) {
                ...
                if (isRideNoMatched) {
                    if (isDistanceMatched) {
                        ...
                        try {
                            costPerKM = returnBO.getVehiCostPerKM(rideNo);
                            discount = returnBO.getCustomerDiscount(rideNo);
                        } catch (SQLException e) {
                            new Alert(Alert.AlertType.ERROR, "Something went wrong\n" + e).show();
                        }
                ...
                if(isDistanceMatched){
                    if(isDateValid){
                        if(isCostValid){
                            ...
                            boolean added;
                            try {
                                added = returnBO.addReturn(new ReturnDTO(returnNo, rideNo, distance, cost, reDate.toString()));
                                if(added){
                            ...
                            } catch (SQLException | ClassNotFoundException | IOException e) { new Alert(Alert.AlertType.ERROR, "Something went wrong\n"+e).show(); }
    ...
    public void btnViewBillOnAction(ActionEvent actionEvent) throws IOException {
        if (!txtReturnNo.getText().isEmpty()) {
            ...
            try{
                returned = returnBO.isReturned(txtReturnNo.getText());
            }catch (SQLException e) {
                new Alert(Alert.AlertType.ERROR, "Something went wrong.\n"+e).show();
            }
            if(returned){
    ...
    public void setLastBill() {
        try {
            String returnNo = returnBO.getLastReturnNo();
            ReturnDTO returnDTOSet = returnBO.getReturn(returnNo);

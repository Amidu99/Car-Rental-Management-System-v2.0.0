...
public class MultipleReturnManageFormController {
    ...
    MultipleReturnsBO mReturnsBO = (MultipleReturnsBO) BOFactory.getBoFactory().getBO(BOFactory.BOTypes.MULTIPLE_RETURN);
    ...
    public void getAllVehicles(String rideNo, String returnNo) {
        try {
            List<String> vehiNos = mReturnsBO.getVehiNos(rideNo);
    ...
    public void btnAddOnAction(ActionEvent actionEvent) {
        ...
        if( cmbVehiNo.getValue() != null && !txtDistance.getText().isEmpty()){
            ...
            if(isDistanceMatched){
                ...
                boolean addDistance;
                try {
                    addDistance = mReturnsBO.addDistance(new RideVehicleDTO(rideNo, vehiNo, distance));

                    if(addDistance){
                         totalDistance = totalDistance + distance;
                         lblTotalDistance.setText(totalDistance+" Km");

                         costPerKm = mReturnsBO.getVehiCostPerKM(vehiNo);
                         subCost = subCost + (distance * costPerKm);
                         lblSubCost.setText("Rs. "+subCost);

                         discount = mReturnsBO.getCustomerDiscount(rideNo);
                ...
                if(obList.isEmpty()) {
                    boolean added;
                    try {
                        added = mReturnsBO.addReturn(new ReturnDTO(returnNo, rideNo, totalDistance, totalCost, LocalDate.now().toString()));

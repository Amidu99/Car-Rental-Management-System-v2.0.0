...
public class MembershipFormController implements Initializable {
    ...
    private void getAll() {
        try {
            ...
            List<MembershipDTO> membList = membershipBO.getAllMembership();

            for(MembershipDTO membershipDTO : membList) {
                obList.add(new MembershipTM(
                        membershipDTO.getCode(),
                        membershipDTO.getType(),
                        membershipDTO.getDiscount(),
                ...
                if (isTypeMatched) {
                    if (isDiscountMatched) {
                        if (isFeeMatched) {
                            ...
                            boolean added = false;
                            try {
                                added = membershipBO.addMembership(new MembershipDTO(code, type, discount, fee));
                            } catch (SQLException | ClassNotFoundException e) {
                ...
                if (isTypeMatched) {
                    if (isDiscountMatched) {
                        if (isFeeMatched) {
                            ...
                            try {
                                updated = membershipBO.updateMembership(new MembershipDTO(code, type, discount, fee));
                            } catch (SQLException | ClassNotFoundException e) {
                                new Alert(Alert.AlertType.ERROR, "Something went wrong:\nNot Updated.\n"+e).show();
                            }
                            if(updated){
        ...
        btnDelete.setOnAction((e) -> {
            ...
            if(!code.isEmpty()){
                ...
                if (result.orElse(no) == yes) {
                    ...
                    try {
                        deleted = membershipBO.deleteMembership(code);
    ...
    public void txtCodeOnAction(ActionEvent actionEvent) {
        ...
        if(!code.isEmpty()){
            ...
            try {
                membershipDTO = membershipBO.searchMembership(code);
            } catch (SQLException | ClassNotFoundException e) {
                new Alert(Alert.AlertType.ERROR, "Something went wrong:\n"+e).show();
            }
            if(membershipDTO != null){
                txtType.setText(membershipDTO.getType());
                txtDiscount.setText(membershipDTO.getDiscount().toString());
                txtFee.setText(membershipDTO.getFee().toString());
            }else{
                new Alert(Alert.AlertType.INFORMATION, "This Membership code is not available").show();

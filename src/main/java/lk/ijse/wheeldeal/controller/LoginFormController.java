...
public class LoginFormController {
    ...
    public void btnSearchOnAction(ActionEvent actionEvent) throws SQLException {
        ...
        if(!empId.isEmpty()){
            try {
                boolean verified = loginBO.checkEmployee(empId);
                if(verified) {
                    empName= loginBO.getEmployeeName(empId);
                    empRole= loginBO.getEmployeeRole(empId);
                    setLabels(empName, empRole);
                    boolean isAvailable = loginBO.isAvailableUser(empId);
    ...
    public void btnCreateOnAction(ActionEvent actionEvent) {
        ...
        if(!userName.isEmpty() && !password.isEmpty() && !passHint.isEmpty()){
            try {
                boolean isAdded = loginBO.addUser(new UserDTO(userID, userName, password, passHint, empID));
                if (isAdded){

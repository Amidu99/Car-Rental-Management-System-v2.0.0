...
public class VehicleFormController implements Initializable {
    ...
    private void generateNextVehicleNo() {
        try {
            String nextNo = vehicleBO.generateNextVehicleNo();
            ...
            if(isVehiNoMatched){
                if(isModelMatched){
                    if(isCostMatched){
                        ...
                        boolean added = false;
                        try {
                            added = vehicleBO.addVehicle(new VehicleDTO(vehiNo, vehiType, model, cost, availability));
                        } catch (SQLException | ClassNotFoundException e) {
                            new Alert(Alert.AlertType.ERROR, "Something went wrong:\nNot Added.\n"+e).show();
                        }
                        if(added){
            ...
            if(isVehiNoMatched){
                if(isModelMatched){
                    if(isCostMatched){
                        ...
                        try {
                            updated = vehicleBO.updateVehicle(new VehicleDTO(vehiNo, vehiType, model, cost, availability));
                        } catch (SQLException | ClassNotFoundException e) {
                            new Alert(Alert.AlertType.ERROR, "Something went wrong:\nNot Updated.\n"+e).show();
                        }
                        if(updated){
        ...
        btnDelete.setOnAction((e) -> {
            ...
            if(!vehiNo.isEmpty()){
                ...
                if (result.orElse(no) == yes) {
                    ...
                    try {
                        deleted = vehicleBO.deleteVehicle(vehiNo);
    ...
    public void txtVehicleNoOnAction(ActionEvent actionEvent) {
        ...
        if(isVehiNoMatched){
            ...
            try {
                vehicleDTO = vehicleBO.getVehicle(txtVehicleNo.getText());
            } catch (SQLException | ClassNotFoundException e) {
                new Alert(Alert.AlertType.ERROR, "Something went wrong:\n"+e).show();
            }
            if(vehicleDTO != null){
                cmbVehicleType.setValue(vehicleDTO.getVehiType());
                txtVehicleModel.setText(vehicleDTO.getModel());
                txtCost.setText(vehicleDTO.getCostPerKM().toString());
                cmbAvailability.setValue(vehicleDTO.getAvailability());
            }else{

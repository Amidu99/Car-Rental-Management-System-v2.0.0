...
public class CustomerFormController implements Initializable {
    ...
    private void generateNextCustomerID() {
        try{
            String nextID = customerBO.generateNextID();
    ...
    private void loadMembershipCodes() {
        try {
            ...
            List<String> codes = customerBO.getMembershipCodes();
    ...
    public void txtCustIDOnAction(ActionEvent actionEvent) {
        ...
        if(!custID.isEmpty()){
            ...
            try {
                customerDTO = customerBO.searchCustomer(custID);
            } catch (SQLException | ClassNotFoundException e) {
                new Alert(Alert.AlertType.ERROR, "Something went wrong:\n"+e ).show();
            }
            ...
            if(customerDTO != null){
                ...
                cmbMembCodes.setValue(customerDTO.getMembCode());
            }else{
                ...
                if (isIDMatched) {
                    if (isNameMatched) {
                        if (isTelMatched) {
                            ...
                            boolean added = false;
                            try {
                                added = customerBO.addCustomer(new CustomerDTO(custId, id, name, tel, membCode));
                            } catch (SQLException | ClassNotFoundException e) {
                                new Alert(Alert.AlertType.ERROR, "Something went wrong:\nNot Added\n" + e).show();
                            }
                            if (added) {
                ...
                if (isIDMatched) {
                    if (isNameMatched) {
                        if (isTelMatched) {
                            ...
                            try {
                                updated = customerBO.updateCustomer(new CustomerDTO(custId, id, name, tel, membCode));
                            } catch (SQLException | ClassNotFoundException e) {
                                new Alert(Alert.AlertType.ERROR, "Something went wrong:\nNot Updated..").show();
                            }
                            if(updated){
        ...
        btnDelete.setOnAction((e) -> {
            ...
            if(!custId.isEmpty()){
                ...
                if (result.orElse(no) == yes) {
                    ...
                    try {
                        deleted = customerBO.deleteCustomer(custId);
